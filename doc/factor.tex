...
Factor does not yet work with a distributed file system (such as CEP2 and CEP3),
...
\begin{enumerate}
\item Collect input bands in a single directory. The MS names can be anything
but must end in ``.MS'' or ``.ms'':
\begin{verbatim}
[/data/factor_input]$ ls
band1.MS         band2.ms         A.MS         b.ms
\end{verbatim}
\item Edit the example parset above to fit your reduction and computing
resource. For example, the parset for a single machine could be:
\begin{verbatim}
