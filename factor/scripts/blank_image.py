...
def main(input_image_file, vertices_file, output_image_file, blank_value='zeros',
    img_format='fits'):
    """
    ...
    blank_value : str, optional
        Value for blanks (one of 'zeros' or 'nans')
    img_format : str, optional
        Format of output mask image (one of 'fits' or 'casa')

    """
    im = pim.image(input_image_file)
    data = im.getdata()
    ...
    if len(outside_ind[0]) > 0:
        if blank_value == 'zeros':
            blank_val = 0.0
        elif blank_value == 'nans':
            blank_val = np.nan
        else:
    ...
    new_im.putdata(data)
    if img_format == 'fits':
        new_im.tofits(output_image_file, overwrite=True)
    elif img_format == 'casa':
        new_im.saveas(output_image_file, overwrite=True)
    else:
